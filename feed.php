...
if($links)
{
	...
	foreach ($links as $key => $link)
	{
		...
		$detail = '';
        $detail .= "<p>".$html->find('table[id=AutoNumber8]',0)->find('tr td span[lang=th] p',0)->innertext."</p>";
        for($i=0;$i<count($html->find('table[id=AutoNumber8]',0)->find('.newslink p'));$i++)
        {
            $detail .= "<p>".$html->find('table[id=AutoNumber8]',0)->find('.newslink p',$i)->innertext."</p>";
        }
